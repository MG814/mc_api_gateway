...
class AddressGatewayViewTests(APITestCase):
    def setUp(self) -> None:
        ...
        self.address_url_get = reverse("address-get-user-address")
    ...
    @patch("api_accounts.views.verify_token")
    @patch("api_accounts.views.forward_request_to_service")
    def test_update_user_address_unauthorized(self, mock_forward_request, mock_verify_token):
        ...
        self.assertEqual(response.json()['message'], 'Unauthorized to update this address')

    @patch("api_accounts.views.forward_request_to_service")
    def test_update_user_address_not_found(self, mock_forward_request):
    ...
    @patch("api_accounts.views.forward_request_to_service")
    def test_create_user_address_success(self, mock_forward_request):
        mock_forward_request.return_value.status_code = status.HTTP_200_OK
        mock_forward_request.return_value.json.return_value = {"user": 1, "locality": "Sample Locality",
                                                               "street": "Sample Street", "zip_code": "12345"}
